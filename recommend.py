...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    lats = [restaurant_location(r)[0]for r in cluster]
    longs = [restaurant_location(r)[1]for r in cluster]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Sxy = sum([(i[0] - mean(xs)) * (i[1] - mean(ys)) for i in both])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    preds = [find_predictor(user, reviewed, f) 
             for f in feature_fns
            ]
    return max(preds, key = lambda x: x[1])[0]
    #return max(preds[0], key = preds[1])
    #max(preds[1], key = preds[0])
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            ratings[rest] = user_rating(user, rest)
        else:
            ratings[rest] = predictor(r)
    return ratings
    # END Question 9
